...
def _agents_refine(description, platform):
    ...
    system = (
        ...
        "EXECUTION quirk (selectors, buttons, popups, navigation) — learning is strictly for HOW to "
        "operate the browser, NEVER for what content to post (that is fixed by this task). Each run the "
        "agent needs an image + a caption, then posts. Choose the IMAGE SOURCE "
        ...
        "prompt derived from the client's direction, save the result to a local file; no mediaId.\n"
        "Then: write a caption (tone/language/length/hashtag+emoji policy per the direction; you may "
        "use the vision model to look at the image first) to a file and post + auto-log/stamp: "
        "`node $GOLOGIN_HELPER post %s /path/caption.txt <image.path> [mediaId]`. Be deterministic, "
