...
if ! grep -q "SOULV13" "$MS" 2>/dev/null; then
cat > "$MS" <<'EOF'
<!-- SOULV13 -->
...
## Products — TWO different brands, NEVER mix (read the briefs FIRST)
- **PeningBot** (peningbot.com, pixel `986352420917190`) = B2B SaaS for SME owners: WhatsApp AI chatbot + hourly ads reporting. Sell time-saved + sales-recovered. RM35-95/mo. Brief: `_products/peningbot.md`.
- **PeningLab** (peninglab.com, pixel `1013990424497184`) = AI UGC content studio for TikTok Shop affiliates: 10 videos in 3 min. Sell cheap fast content. RM35-200/mo. Brief: `_products/peninglab.md`.
Each = its own CBO, pixel, creatives, voice, visual identity. Never share creative between them.
...
- **agency-architecture** = the full A-Z **17-agent** system (only the Ad Builder writes ads; **Ad Builder runs 01:00 MYT daily**). Read it to know your role + the `_shared/` hand-offs (no overlap, no gaps).
...
- **Budget:** RM 4/day PeningBot + RM 4/day PeningLab (RM 8/day total, MYR). Create everything PAUSED; WhatsApp the owner to approve before any spend.
